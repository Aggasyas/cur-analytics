# -*- coding: utf-8 -*-
"""
Индекс-архив всех сводок ЦУР с навигацией год→месяц→день.
Самодостаточный HTML. Каждый день — карточка с оценкой и ключевыми числами.
"""
...
try:
    from metrics_cur import load_history, compare, verdict
except Exception:
    from .metrics_cur import load_history, compare, verdict
...
# По каким показателям оцениваем «спокойнее/напряжённее»
VERDICT_KPI = ["total", "problems", "urgent"]
...
<title>Аналитика ЦУР · архив</title>
<style>
:root{{--bg:#f4f6f9;--card:#fff;--ink:#1a2230;--muted:#6b7686;--line:#e3e8ef;
--brand:#15507a;--brand-soft:#e8f1f8;--red:#c0392b;--green:#1f7a4d;--amber:#c77700}}
...
header.doc{{background:linear-gradient(135deg,#15507a,#1d6ea3);color:#fff;border-radius:14px;padding:22px 26px;box-shadow:0 6px 20px rgba(21,80,122,.2)}}
header.doc h1{{margin:0 0 4px;font-size:23px}}
header.doc .sub{{opacity:.92;font-size:14px}}
...
.row.latest{{border-color:#bcd6ec;box-shadow:0 0 0 2px var(--brand-soft)}}
.dt{{min-width:185px}}
...
.mo .list{{margin:0;padding:10px 12px;display:flex;flex-direction:column;gap:10px}}
</style></head><body><div class="wrap">"""]

    P.append(f"""<header class="doc">
<h1>Аналитика обращений ЦУР · архив</h1>
<div class="sub">Центр управления регионом · г.о. Солнечногорск · всего дней в архиве: {len(rows)}</div>
</header>""")
...
<div class="dt"><div class="d">{esc(long_date(d))}</div><div class="w">{esc(wday(d))}</div></div>
<div class="vd {vd_cls}">{esc(vd_txt)}</div>
<div class="nums">
<span class="n">Всего <b>{esc(r.get('total',0))}</b></span>
<span class="n">Проблем <b>{esc(r.get('problems',0))}</b></span>
<span class="n urg">Аварий/инц. <b>{esc(urg)}</b></span>
<span class="n">В админ. <b>{esc(r.get('own_admin',0))}</b></span>
</div>
<div class="links">
...
<a href="analytics-{esc(d)}.html">Аналитика</a>
    ...
    P.append('<p class="foot">Архив формируется автоматически из ежедневных выгрузок тепловой карты ЦУР.</p>')
...
if __name__ == "__main__":
    ...
    ap.add_argument("history", nargs="?", default="cur_history.jsonl")
